...
function getUserInfo(username = 'davidiamyou') {
  return axios.get(`https://api.github.com/users/${username + param}`)
}

function getRepos(username = 'davidiamyou') {
...
export async function battle (players) {
  try {
    const data = await Promise.all([getPlayersData(players[0]), getPlayersData(players[1])])
